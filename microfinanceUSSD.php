		...
		if($level==0 || $level==1){
			...
			switch ($userResponse) {
			    case "":
			        if($level==0){
						...
						$response = "CON Welcome to CASH AT HAND BANKING, " . $userAvailable['name']  . ". Choose a service.\n";
						$response .= " 1. Get a call from customer care.\n";
			    ...
			    case "0":
			        if($level==0){
			        	...
			        	//Serve our services menu
						$response = "CON Welcome to CASH AT HAND, " . $userAvailable['username']  . ". Choose a service.\n";
						$response .= " 1. Get a call from customer care.\n";
						$response .= " 2. Deposit\n";
						$response .= " 3. Withdraw\n";
						$response .= " 4. Send Money\n";							
			    ...
			    case "1":
			        if($level==1){
			        	//9d. Call the user and bridge to a sales person
			          	...
			          	//Make a call
			         	$from="+25429201358"; $to=$phoneNumber;
			    ...
			    case "2":
			    	if($level==1){
						...
						$response .= " 1. 500 Shillings.\n";
						$response .= " 2. 1000 Shillings.\n";
						$response .= " 3. 5000 Shillings.\n";
			    ...
			    case "4":
			    	if($level==1){
						...
						$response .= " Enter a valid phonenumber (like 0729201358)\n";
			  			// Print the response onto the page so that our gateway can read it
			  			header('Content-type: text/plain');
 			  			echo $response;	
						...
						$newBal -= 500;
						    ...
						    if($newBal > 0){					    	
								$gateway = new AfricasTalkingGateway($username, $apiKey);
								$productName  = "CASH AT HAND Payments"; $currencyCode = "KES";
								$recipient1   = array("phoneNumber" => $phoneNumber,"currencyCode" => "KES", "amount"=> 500,"metadata"=>array("name"=> "Clerk","reason" => "May Salary"));
			    ...
			    case "6":
			    	if($level==1){
			    		//9e. Ask how much and Launch the Mpesa Checkout to the user
						$response = "CON How much are you depositing?\n";
						$response .= " 4. 500 Shilling.\n";
						$response .= " 5. 1000 Shillings.\n";
						$response .= " 6. 5000 Shillings.\n";
			...
			switch ($level){
					...
					switch ($userResponse) {
					    case "1":
							...
							if($newBal > 0){
						    	...
						    	$response .= " KES 500/- shortly... \n";

								//Declare Params
								$gateway = new AfricasTalkingGateway($username, $apikey);
								$productName  = "CASH AT HAND";
								$currencyCode = "KES";
								$recipient   = array("phoneNumber" => "".$phoneNumber."","currencyCode" => "KES","amount"=>500,"metadata"=>array("name"=>"Client","reason" => "Withdrawal"));
					    ...
					    case "2":
							...
							if($newBal > 0){					    
						    	//Alert user of incoming Mpesa cash
						    	$response = "END We are sending your withdrawal of\n";
						    	$response .= " KES 1000/- shortly... \n";

								//Declare Params
								$gateway = new AfricasTalkingGateway($username, $apikey);
								$productName  = "CASH AT HAND";
								$currencyCode = "KES";
								$recipient   = array("phoneNumber" => "".$phoneNumber."","currencyCode" => "KES","amount"=>1000,"metadata"=>array("name"=>"Client","reason" => "Withdrawal"));
								$recipients  = array($recipient);
								//Send B2c
								try {$responses = $gateway->mobilePaymentB2CRequest($productName, $recipients);}
								catch(AfricasTalkingGatewayException $e){echo "Received error response: ".$e->getMessage();}
					    ...
					    case "3":
							...
							if($newBal > 0){					    
								...
								//Declare Params
								$gateway = new AfricasTalkingGateway($username, $apikey);
								$productName  = "CASH AT HAND PAYMENT";
								$currencyCode = "KES";
								$recipient   = array("phoneNumber" => "".$phoneNumber."","currencyCode" => "KES","amount"=>500,"metadata"=>array("name"=>"Client","reason" => "Withdrawal"));
								$recipients  = array($recipient);
								//Send B2c
			    ...
			    case 11:
					...
					$response = "END We are sending KES 500/- \n";
					...
					if($newBal > 0){
		            	...
		            	$message    = "We have sent 500/- to".$userResponse." If this is a wrong number the transaction will fail.
			    ...
			    case 12:
					...
					switch ($userResponse) {
					    case "4":
						    //End session
					    	$response = "END Kindly wait 1 minute for the Checkout. You are repaying 500/-..\n";
					    	// Print the response onto the page so that our gateway can read it
					  		header('Content-type: text/plain');
		 			  		echo $response;	
					    ...
					    case "5":
						    //End session
					    	$response = "END Kindly wait 1 minute for the Checkout. You are repaying 1000/-..\n";
					    	// Print the response onto the page so that our gateway can read it
					  		header('Content-type: text/plain');
		 			  		echo $response;	
					    ...
					    case "6":
						    //End session
					    	$response = "END Kindly wait 1 minute for the Checkout. You are repaying 5000/-..\n";
					    	// Print the response onto the page so that our gateway can read it
					  		header('Content-type: text/plain');
		 			  		echo $response;	
		...
		if($userResponse==""){
			...
			switch ($level) {
			    ...
			    case 2:
		        	...
		        	$sql11e = "INSERT INTO `session_levels`(`session_id`,`phoneNumber`,`level`) VALUES('".$sessionId."','".$phoneNumber."',0)";
		        	$db->query($sql11e);  

					//11f. Serve the menu request for name
					$response = "END You have been successfully registered to CASH AT HAND. Dial *384*303030# to choose a service.";
